...
public class DiGraph implements Graph {
	...
	public List<Node> breadthFirstSearch(Node startNode){
		//asi lo tenia asi lo entregue , 100 puntos, solo voy a practicar
//		if(startNode == null || nodes == null || nodes.isEmpty()) return null;
//		LinkedList<Node> nodeList = new LinkedList<Node>();
//		resetState();
//		Queue<Node> q = new LinkedList<Node>();
//		Node it =  startNode;
//		nodeList.add(it);
//		q.addAll(getAdjacentNodes(it));
//		
//
//		while(!q.isEmpty() && it.predecessor == null  ){
//			//System.out.print("Nachbarn von Node " + it.id + " : ");
//			it.predecessor = q.poll();
//			it= it.predecessor;
//			ArrayList<Node> temp = new ArrayList<Node>(it.getAdjacentNodes());
//			for(int i = 0; i< temp.size(); i++){
//				if(q.contains(temp.get(i))) continue;
//				q.add(temp.get(i));
//			}
...
//			
//		}
//		if(!nodeList.contains(it)) nodeList.add(it);
////		System.out.println( "BFS : " +nodeList.toString());
		LinkedList<Node> nodeList = new LinkedList<Node>();
		...
		q.add(startNode);
		besuchteKnoten.add(startNode);
		
		while(!q.isEmpty()){
			Node n = q.poll();
			for(Node k : n.getAdjacentNodes()){
				if(!besuchteKnoten.contains(k)){
					nodeList.add(k);
					q.add(k);
					besuchteKnoten.add(k);
				}
			}
		}
	...
	public List<Node> depthFirstSearch(Node startNode){
		//creo que los errores son porque hacen el test de BFS y despues me cagan la vuelta
		if(startNode == null || nodes == null|| nodes.isEmpty()) return null;
		LinkedList<Node> nodeList = new LinkedList<Node>();
		resetState();
		...
		Node its = startNode;
		s.addAll(getAdjacentNodes(its));
		visitedNodes.add(its);
		
		while(!s.isEmpty() && its.predecessor == null  ){ 
			ArrayList<Node> temp = new ArrayList<Node>(its.getAdjacentNodes());
			for(int i = 0; i< temp.size(); i++){
				if(visitedNodes.contains(temp.get(i))) continue;
				
				s.push(temp.get(i));
			
			}		
			nodeList.add(its);
			its.predecessor = s.pop();
			visitedNodes.add(its);
			its= its.predecessor;
			
			
			//System.out.println(its.predecessor);
			
//			System.out.println(s.toString());
//			System.out.println(s.peek().toString());
			
		}
		if(!nodeList.contains(its)) nodeList.add(its);
//		System.out.println(s.toString());
	//
//		System.out.println(" DFS : "+nodeList.toString());
