...
//	private final Color LABEL_COLOR = new Color(0x00aa00);
	...
	public void addEdge(VisualNode start, VisualNode target, double weight) {
...
//		graphicsMain.fillRect(0, 0, this.getWidth(), this.getHeight());
	...
	private void drawEdgeLabel(Graphics g, Point center, String label, int color) {
		...
		if (label.length() >= 2){
			g.setColor(new Color(0, 0, 0));
			g.fillRect(wX -4, wY - 13, 74, 26);
			g.setColor(COLORS[color]);
			g.fillRect(wX - 2, wY - 11, 70, 22);
		}else{
			g.setColor(new Color(0, 0, 0));
			g.fillRect(wX -4, wY - 13, 66, 26);
			g.setColor(COLORS[color]);
			g.fillRect(wX - 2, wY - 11, 62, 22);
		}
		if(color == 2){
