...
public class TokenGridGraph extends GridGraph {
	...
	ArrayList<Node> computeTour(CellNode startNode) {
		...
		//arrayList tokens soriert
		HeuristicEuclidean h = new HeuristicEuclidean((CellNode) startNode);
		for(CellNode cn : tokens){
			cn.distance = Integer.MAX_VALUE;
		}
		ArrayList<CellNode> tokensSortiert = new ArrayList<CellNode>();
		
		PriorityQueue<CellNode> sortiert = new PriorityQueue<CellNode>(h);
		sortiert.addAll(tokens);
		sortiert.remove(startNode);
		
		
		
		
		while(!sortiert.isEmpty()){
			tokensSortiert.add(sortiert.poll());
		}

		//Iterate over the tokens in given order
		for(CellNode token : tokensSortiert)
		{
			if(tokenBefore != null)
			{
